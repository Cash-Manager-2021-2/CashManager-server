package eu.epitech.cashmanager31.cashmanager;

import eu.epitech.cashmanager31.cashmanager.model.Item;
import eu.epitech.cashmanager31.cashmanager.repository.ItemRepository;
...
public class CashHandler implements WebSocketHandler {
    ...
    private static Map<Identity, Basket> Baskets = new HashMap<>();
    ...
    private final Action[] a_article = {
            (msg, s) -> {
                if (msg.parameters[0].equals("add")) {
                    ...
                    try {
                        ...
                        Baskets.get(Identity.getId(s)).addArticle(art);
                        var ar = Baskets.get(Identity.getId(s)).getArticle(art);
            ...
            (msg, s) -> {
                if (msg.parameters[0].equals("remove")) {
                    ...
                    try {
                        var i_qty = Integer.parseInt(qty);
                        var art = new Article(name, i_qty, itemRepository);
                        Baskets.get(Identity.getId(s)).reduceArticle(art);
                        var ar = Baskets.get(Identity.getId(s)).getArticle(art);
                        return notify("article " + ar.toString(), s);
                    } catch (NoSuchElementException e) {
                        return error(ErrorStatus.BAD_ARTICLE);
            ...
            (msg, s) -> {
                if (msg.parameters[0].equals("clear")) {
                    Baskets.get(Identity.getId(s)).clear();
                    return notify(Baskets.get(Identity.getId(s)).toString(), s);
                }
                return null;
            },
            (msg, s) -> {
                if (msg.parameters[0].equals("list") ||
                    ...
                    return message(Baskets.get(Identity.getId(s)).toString());
                }
                return null;
            },
    };
    private final Action[] a_basket = {
            (msg, s) -> {
                if (msg.parameters[0].equals("list") ||
                    msg.parameters[0].equals("get")) {
                    return message(Baskets.get(Identity.getId(s)).toString());
    ...
    private final Action[] commands = {
            (msg, s) -> {
                if (msg.command.equals("password")) {
                    ...
                    if (id.valid()) {
                        ...
                        if (!Baskets.containsKey(Identity.getId(s)))
                            Baskets.put(Identity.getId(s), new Basket());
            ...
            (msg, s) -> {
                if (msg.command.equals("basket")) {
                    if (msg.parameters.length < 1)
                        return error(ErrorStatus.BAD_ARGUMENTS);
                    Pair<Boolean, String> ret;
                    for (Action m : a_basket)
                        if ((ret = m.action(msg, s)) != null)
                            return ret;
                    return error(ErrorStatus.UNKNOWN_ACTION);
                }
